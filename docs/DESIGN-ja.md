...
- Cleaner と同じ「触らない」集合をインポート側にも適用する。WordPress コアオプション（`CoreOptions`）は決して書き込まず、Optrion 自身の名前空間（プラグインオプション `optrion_*`、および manifest テーブルが管理する検疫リネーム名前空間）も拒否する。スキップしたエントリはサマリーで報告される
